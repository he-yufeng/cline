...
## Bundling Skills in a Plugin

Use `api.registerSkill()` when a plugin should install reusable instructions along with its tools or hooks. The plugin must declare the `skills` capability.
...
const plugin: AgentPlugin = {
  ...
  setup(api) {
    api.registerSkill({
      name: "review",
      description: "Review a code change with project-specific checks.",
      instructions: [
        "Inspect the current diff.",
        "Check for missing tests, behavior changes, and migration risk.",
        "Return findings first, then any open questions.",
      ].join("\n"),
    })
  },
}
...
When the plugin is installed, end users get the bundled skill through the same runtime path as local `SKILL.md` files.
